...
for y in range(len(ds_matrix[0])): #iterate over rows
    row = []
...
for img in interpolated:
    result = Image.fromarray(img.astype(np.uint8)) #convert to image
    result.save(f"Results/company_emission_{year}_1000x1000.tif")  #save as tif
    year +=1
